...
const armorStatServices = {
    ...
    refreshArmorPower: async (armorId) => {
        try {
            ...
            console.error(error);
            throw new Error('Failed to refresh armor');
        }
    },
    editArmorBlock: async (armorId, value) => {
        try {
            ...
            const newStats = Object.assign(Object.assign({}, statsObject), { currentPower: statsObject.currentBlock + Number(value) });
            await prisma.armor.update({
                where: {
                    id: Number(armorId),
                },
    ...
    refreshArmorBlock: async (armorId) => {
        try {
            ...
            const statsObject = armor.stats;
            const newStats = Object.assign(Object.assign({}, statsObject), { currentPower: statsObject.block });
            await prisma.armor.update({
                where: {
                    id: Number(armorId),
                },
            ...
            console.error(error);
            throw new Error('Failed to refresh block points');
        }
    },
};
