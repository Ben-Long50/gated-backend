...
const itemStatServices = {
    editItemAmmo: async (itemId, value, userId) => {
        var _a, _b;
        try {
            ...
            const newStats = typeof item.stats === 'object'
                ? Object.assign(Object.assign({}, item.stats), { currentAmmoCount: newAmmoValue }) : {};
    ...
    reloadItem: async (itemId, userId) => {
        var _a, _b;
        try {
            const item = await prisma.item.findUnique({
                where: {
            ...
            const newStats = typeof item.stats === 'object'
                ? Object.assign(Object.assign({}, item.stats), { currentAmmoCount: statsObject.magCapacity, currentMagCount: statsObject.currentMagCount - 1 }) : {};
            await prisma.item.update({
                where: {
                    id: Number(itemId),
                },
    ...
    refreshAmmo: async (itemId, userId) => {
        var _a, _b;
        try {
            const item = await prisma.item.findUnique({
                where: {
            ...
            const statsObject = getStatsObject(item);
            const newStats = typeof item.stats === 'object'
                ? Object.assign(Object.assign({}, item.stats), { currentAmmoCount: statsObject.magCapacity, currentMagCount: statsObject.magCount - 1 }) : {};
            await prisma.item.update({
                where: {
                    id: Number(itemId),
                },
    ...
    editItemPower: async (itemId, value, userId) => {
        ...
        try {
            ...
            const newStats = typeof item.stats === 'object'
                ? Object.assign(Object.assign({}, item.stats), { currentPower: newPowerValue }) : {};
            await prisma.item.update({
                where: {
                    id: Number(itemId),
                },
    ...
    refreshItemPower: async (itemId, userId) => {
        ...
        try {
            ...
            const statsObject = getStatsObject(item);
            const newStats = typeof item.stats === 'object'
                ? Object.assign(Object.assign({}, item.stats), { currentPower: statsObject.power }) : {};
            await prisma.item.update({
                where: {
                    id: Number(itemId),
                },
